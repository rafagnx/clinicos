...
export const blockedDaysApi = {
    ...
    list: async ({ professionalId, startDate, endDate }) => {
        try {
            ...
            if (professionalId && String(professionalId) !== 'undefined' && String(professionalId) !== 'null') {
                params.professionalId = String(professionalId);
            }

            const response = await api.get('/blocked-days', { params });
