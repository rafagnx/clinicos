...
export const base44 = {
    ...
    auth: {
        me: async () => {
            ...
            if (user) {
                // --- PROACTIVE TOKEN SLIMMING ---
                // If user metadata is huge, it causes ERR_CONNECTION_CLOSED on Render.
                try {
                    const meta = user.user_metadata;
                    if (meta && Object.values(meta).some(v => v && typeof v === 'string' && v.length > 1000)) {
                        console.warn("🛡️ [Auth] Astronomical metadata detected. Clearing image junk...");
                        await supabase.auth.updateUser({
                            data: { image: "", avatar_url: "", picture: "", photo_url: "", avatar: "" }
                        });
                        // Allow a small delay or proceed; the next backend call will attempt to use the token
                    }
                } catch (e) { console.warn("Slimming failed", e); }
    ...
    storage: {
        upload: async (file, path = 'images') => {
            try {
                ...
                if (error) {
                    // Fallback to Base64 if bucket doesn't exist yet, but warn
                    console.error("[Storage] Error uploading to bucket:", error);
                    if (error.message.includes("not found")) {
                        console.warn("⚠️ [Storage] Bucket 'images' not found in Supabase. Please create it.");
                    }
